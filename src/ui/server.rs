...
struct UiState {
    ...
    studio_events: Arc<tokio::sync::Mutex<HashMap<String, broadcast::Sender<String>>>>,
}
...
pub async fn start(port: u16, open_browser: bool) -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route(
            "/api/studio/sessions",
            get(api_studio_list_sessions).post(api_studio_create_session),
        )
        .route(
            "/api/studio/sessions/{id}",
            get(api_studio_get_session).delete(api_studio_delete_session),
        )
        .route(
            "/api/studio/sessions/{id}/images",
            post(api_studio_upload_images),
        )
        .route(
            "/api/studio/sessions/{id}/start",
            post(api_studio_start_session),
        )
        .route("/api/studio/sessions/{id}/stream", get(api_studio_stream))
...
/// POST /api/studio/sessions — Create a new studio session.
async fn api_studio_create_session(Json(req): Json<CreateSessionRequest>) -> impl IntoResponse {
    ...
    match tokio::task::spawn_blocking({
        ...
        move || {
            ...
            // Create the session directory for images
...
/// GET /api/studio/sessions — List all studio sessions.
async fn api_studio_list_sessions() -> impl IntoResponse {
    match tokio::task::spawn_blocking(|| {
        let db = Database::open()?;
...
/// GET /api/studio/sessions/:id — Get session details + events + results.
async fn api_studio_get_session(Path(id): Path<String>) -> impl IntoResponse {
    match tokio::task::spawn_blocking(move || {
        let db = Database::open()?;
        let session = db
...
/// DELETE /api/studio/sessions/:id — Delete a session.
async fn api_studio_delete_session(Path(id): Path<String>) -> impl IntoResponse {
    match tokio::task::spawn_blocking(move || {
        let db = Database::open()?;
...
/// POST /api/studio/sessions/:id/images — Upload images (multipart/form-data).
async fn api_studio_upload_images(
    Path(id): Path<String>,
...
/// POST /api/studio/sessions/:id/start — Start agent execution.
async fn api_studio_start_session(
    State(state): State<UiState>,
    Path(id): Path<String>,
) -> impl IntoResponse {
    // Get session from DB
    ...
    // Create event broadcast channel for this session
    ...
    // Resolve image paths
    let root = modl_root();
    let image_paths: Vec<PathBuf> = input_images
        .iter()
        .map(|p| root.join(p))
        .filter(|p| p.exists())
        .collect();
...
/// GET /api/studio/sessions/:id/stream — SSE agent events.
async fn api_studio_stream(
    State(state): State<UiState>,
    Path(id): Path<String>,
) -> Sse<impl Stream<Item = std::result::Result<Event, Infallible>>> {
    // Try to get existing event sender
    let rx = {
